...
public:
  ...
  bool is_active() { return gtk_window_is_active(dialog); };
  void set_dialog(GtkWindow *new_dialog_window) { dialog = new_dialog_window; }
};
...
void can_button_clicked(GtkButton *button, gpointer user_data) {
  // parameter "button" ignore, is only for jumping over the automatic insersion
  // of GTK
...
// Logic acc_button
//
// for root always uid=0(root) gid=0(root) groups=0(root)
char *get_file_owner_id(char *path) {
  g_print("path in : %s", path);
...
void acc_button_clicked(GtkButton *button, gpointer user_data) {
  ...
  std::string command = std::string("touch ").append(full_path);

  char *user_name = get_file_owner_id(path_literal);
  g_print("user_name = %s, %s, \n", user_name, full_path);

  if (strcmp(user_name, "root") == 0) {
    std::string root_command = std::string("pkexec ").append(command);
    system(root_command.c_str());
    return;
  } else {
...
GtkWindow *dialog_factory(GtkWindow *parent, char *path) {
  ...
  g_signal_connect(can_button, "clicked", G_CALLBACK(can_button_clicked),
                   window);

  CurrentLocation *data = new CurrentLocation(entry, path);

  g_signal_connect(acc_button, "clicked", G_CALLBACK(acc_button_clicked), data);
...
void on_click(NautilusMenuItem *item, gpointer user_data) {
  ...
  // 'user_data' is whatever pointer you passed during registration (e.g., the
  // folder path)
  char *path = (char *)user_data;
  GtkWindow *parent_window = gtk_application_get_active_window(
...
extern "C" GList *get_background_items(NautilusMenuProvider *provider,
                                       NautilusFileInfo *info) {

  // Create your item
  NautilusMenuItem *item = nautilus_menu_item_new(
      "CreateFileExtension::NewFileAction", "Create File...",
      "Create a new file", "document-new");

  char *uri = nautilus_file_info_get_uri(info);

  g_signal_connect_data(item, "activate", G_CALLBACK(on_click), uri,
